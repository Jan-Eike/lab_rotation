...
def main():
    ...
    plot_all_channels(k_nearest_time_series, labvitals_list_test, best_paths, best_distances, nn_with_false_label)


def plot_explain(nearest_neighbor, labvitals_list_test, best_paths, nn, test_point, nn_with_false_label):
    ...
    for channel in range(number_of_channels-20):
        ...
        if plot_dtw:
            ...
            path, d = metrics.dtw_path(time_series_1, time_series_nn_with_false_label)
            fig2, ax2 = dtwvis.plot_warping(time_series_1, time_series_nn_with_false_label, path)
            ...
            ax2[1].set_title("nearest neighbor with false label", y=1, pad=-14)
            plt.show()


def plot_all_channels(k_nearest_time_series, labvitals_list_test, best_paths, best_distances, nn_with_false_label):
    ...
    for test_point in test_points:
        ...
        for k, nearest_neighbor in enumerate(k_nearest_time_series[test_point][0]):
            plot_explain(nearest_neighbor, labvitals_list_test, best_paths, k, test_point, nn_with_false_label[test_point])
